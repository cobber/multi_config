...
# TODO: Riehm 2011-02-18 if buffering is turned on, then it should continue
#                           buffering until the application tells it to stop buffering.
#                           That way, a full stack of observers can be set up and all filtering etc.
#                           will work properly when the buffer is emptied.
...
sub import
    ...
    while( my $symbol = shift @symbols )
        ...
        $case     = lc( $1 )                                    if $symbol =~ /^-(upper|lower)(case)?$/i;
...
#  @param   {convert_case_to}   convert to a specific upper/lower case?
#  @return  <none>
sub export_notification_function {
    ...
    @name_parts{qw( is prefix event suffix )} =
        map { $param{convert_case_to} eq 'upper' ? uc( $_ ) : $_ }
        map { $param{convert_case_to} eq 'lower' ? lc( $_ ) : $_ }
...

    {
        ...
        *{$notify_function} = sub {
        ...
        # return 1 or 0 if the event would be propagated to observers
        *{$check_function} = sub {
...
## @fn      notify( <message>, %user_data )
#  @brief   send a message to all observers - this should not be called directly!
#  @detail  a notification object will be created and distributed to all
#           registered observers in reverse stack order
#           See Notifications::Notification for more details
#  @param   <message>   the first parameter is the message to be printed
#  @param   %user_data  all following parameters must be in name => value pairs for generic handling
#  @return  <none>
sub notify
    {
    my %param = @_;

    my @caller = $param{caller} ? @{$param{caller}} : (caller(0))[0..3];
    my $notification = Notifications::Notification->new(
        'message'            => $param{message},
        'event'              => $param{event},
        'source_package'     => $caller[0],
        'source_file'        => $caller[1],
        'source_line_number' => $caller[2],
        'source_function'    => $caller[3],
        'timestamp'          => $param{timestamp} || time(),
        'user_data'          => $param{user_data} || {},
        );

    # TODO: Riehm [2011-02-12] optimise for specific events / packages
...
sub start_buffer {
    ...
    $buffer->start();   # pushes buffer to the top of the stack
